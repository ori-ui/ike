use crate::{
    Canvas, CornerRadius, Offset, Size,
    canvas::{Fonts, Paint, Shader},
    math::{Affine, Rect},
    text::Paragraph,
};

pub struct SkiaFonts {
    collection: skia_safe::textlayout::FontCollection,
}
...
impl SkiaFonts {
    ...
    fn build_paragraph(&mut self, paragraph: &Paragraph) -> skia_safe::textlayout::Paragraph {
...
pub struct SkiaCanvas<'a> {
    ...
    fonts: &'a mut SkiaFonts,
}

impl<'a> SkiaCanvas<'a> {
    pub fn new(canvas: &'a skia_safe::Canvas, fonts: &'a mut SkiaFonts) -> Self {
        Self { canvas, fonts }
    }
