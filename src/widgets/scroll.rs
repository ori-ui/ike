...
use crate::{
    ...
    LayoutCx, Padding, Paint, Painter, Point, PointerEvent, PointerPropagate, Rect, Size, Space,
    Transition, Transitioned, Update, UpdateCx, Widget, WidgetMut,
};
...
impl Widget for Scroll {
    fn layout(&mut self, cx: &mut LayoutCx<'_>, painter: &mut dyn Painter, space: Space) -> Size {
        ...
        let child_space = Space {
            min: self.axis.pack_size(min_major, min_minor - self.bar_width),
            max: self.axis.pack_size(
                f32::INFINITY, // allow as much content as possible
                max_minor - self.bar_width,
            ),
        };
    ...
    fn draw(&mut self, cx: &mut DrawCx<'_>, canvas: &mut dyn Canvas) {
        ...
        let bar = self.bar_rect(cx.size());

        canvas.draw_rect(bar, self.bar_corner_radius, &self.bar_paint);
        canvas.draw_border(
            bar,
    ...
    fn on_pointer_event(&mut self, cx: &mut EventCx<'_>, event: &PointerEvent) -> PointerPropagate {
        match event {
            ...
            PointerEvent::Scroll(event) if cx.has_hovered() => {
                let mut scroll = *self.scroll - self.axis.unpack_offset(event.delta).0;
                scroll = scroll.clamp(0.0, self.overflow());
